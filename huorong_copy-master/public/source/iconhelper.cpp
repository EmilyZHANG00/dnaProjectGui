...
bool IconHelper::Load(QString strTTF)
{
    int nFontId = QFontDatabase::addApplicationFont(strTTF);
...
void IconHelper::SetIcon( QPushButton *btn, QChar iconCode, uint dwSize /*= 26*/)
{
    ...
    btn->setCursor(Qt::PointingHandCursor); //设置鼠标样式
}
...
QPixmap IconHelper::GetDragChnPixmap( QString strText )
{
    ...
    int nW = cs.size(0,strText).width() + nSpace * 2;
