...
void CSlider::mouseMoveEvent(QMouseEvent *event)
{
    ...
    {
        QPoint pt;
        int pos = GetValueByPos(event->pos());
        pt.setX(event->pos().x() - m_rcBg.left());
        pt.setY(event->pos().y());

        emit SignalHover(pt, pos);
        // qDebug() << event->pos() << pt << pos;
    }
    else
    {
        emit SignalLeave();
...
void CSlider::UpdateItemList()
{
    ...
    m_rcBg.setLeft(m_rcBg.left() + 15);
    m_rcBg.setRight(m_rcBg.right() - 15);
...
int CSlider::GetValueByPos(QPoint ptPos)
{
    int nValue = 0;
    double nStepW = (double)m_rcBg.width() / (m_nTotal / m_nStep);
    nStepW /= 2;
