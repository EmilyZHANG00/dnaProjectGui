...
RollTitles::RollTitles(QWidget *parent) : QWidget(parent)
{
    m_nLeftOffset = 0;
    m_nTimerId = -1;
...
void RollTitles::timerEvent(QTimerEvent *event)
{
    if(event->timerId() == m_nTimerId && isVisible())
    {
        ...
        QFont ft = font();
        QFontMetrics fm(ft);
        int txtWidth = fm.size(0,m_strTitle).width();
        int spaceWidth = fm.size(0,strSpace).width();
...
void RollTitles::UpdateRollingState()
{
    ...
    QFont ft = font();
    QFontMetrics fm(ft);
    int nW = fm.size(0,m_strTitle).width();
